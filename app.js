...
let demandCalls=null,demandBusy=false;
function demandMailTopics(emails){
 ...
 for(const mail of emails){
  ...
  for(const evidence of mail.demoEvidence||[]){
   ...
   const title=String(mail.subject||'Без темы').trim(),groupKey=evidence.product+'|'+title.toLowerCase();
...
function renderDemand(){
 ...
 document.getElementById('demandStatus').textContent='Письма: '+fmt(registryData.length)+' · Campaign сопоставлен: '+fmt(matched)+' · звонки: '+esc(meta.period||'весь доступный период');
 ...
 document.getElementById('demandDemo').innerHTML=topics.length?`<table><thead><tr><th>Тема / продукт</th><th>Зелёные</th><th>Жёлтые</th><th>Красные</th><th>Основание</th></tr></thead><tbody>${topics.map(t=>`<tr><td><b>${esc(t.title)}</b><small>${esc(t.product)}</small></td><td><b>${fmt(t.green)}</b></td><td>${fmt(t.yellow)}</td><td>${fmt(t.red)}</td><td><details><summary>Метки: ${t.letters.length}</summary>${t.letters.map(m=>`<p>${esc(m.date)} · ${esc(m.campaign)}<br>${safeLink(m.url,'Статистика DEMO')} · ${safeLink(m.sendsay,'Письмо в Sendsay')}</p>`).join('')}</details></td></tr>`).join('')}</tbody></table>`:'<p>Нет сопоставленных тем по выбранному запросу.</p>';
