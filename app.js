...
function renderDemand(){
 ...
 document.getElementById('demandStatus').textContent='Письма: '+fmt(registryData.length)+' · Campaign сопоставлен: '+fmt(matched);
 document.getElementById('demandSummary').innerHTML=`<div><b>${fmt(all.filter(t=>t.green>0).length)}</b><span>тем писем с зелёными демо</span></div><div><b>${fmt(all.reduce((sum,t)=>sum+t.green,0))}</b><span>зелёных демо по этим темам</span></div><div><b>${fmt(all.length)}</b><span>тем с сопоставленной статистикой</span></div>`;
