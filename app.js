...
function renderNewsMaterials(){
 ...
 document.getElementById('newsMaterialStatus').textContent=`Проверено писем: ${done} из ${mails.length}${errors?' · Ошибок: '+errors:''}. ${done<mails.length?'Результат пока неполный. ':''}Период: ${select.value?'неделя '+select.value:'все доступные недели'}. Без демо — нет результата в доступном факте за этот период; переходы на отдельные материалы здесь не учитываются.`;
